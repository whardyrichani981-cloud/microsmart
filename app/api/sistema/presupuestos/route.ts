...
export async function GET() {
  let items = await getPresupuestos()
  ...
  if (sinNumero.length > 0) {
    ...
    items = await getPresupuestos()
  }
  const nextNum = await getNextPresupuestoNum()
  return NextResponse.json({ items, nextNum })
}

export async function POST(req: NextRequest) {
  const data = await req.json()
  const item = await addPresupuesto(data)
  return NextResponse.json(item, { status: 201 })
}

export async function PUT(req: NextRequest) {
  const { id, ...data } = await req.json()
  const item = await updatePresupuesto(id, data)
