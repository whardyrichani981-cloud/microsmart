...
function ReparacionesTab({ data, q }: { data: GremioData; q: string }) {
  ...
  return (
    <>
      {q && <div style={{ marginBottom: 12, fontSize: 12, color: '#7c85a2' }}>{total} resultado{total !== 1 ? 's' : ''}</div>}
            ...
            {filtered.map((item, i) => {
              ...
              return (
                <tr key={i} style={{ background: i % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                  <td style={{ ...td, color: '#e2e8f0', fontWeight: 500 }}>
                    <Highlight text={item.name} q={q} />
                    {item.notas && <span style={{ display: 'block', fontSize: 11, color: '#7c85a2', marginTop: 2 }}>{item.notas}</span>}
                  </td>
      ...
      {filtered.length === 0 ? <NoResults /> : (
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filtered.map((item, i) => (
              <tr key={i} style={{ background: i % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                <td style={{ ...td, color: '#e2e8f0', fontWeight: 500 }}><Highlight text={item.name} q={q} /></td>
                <td style={{ ...td, textAlign: 'right', color: '#818cf8', fontWeight: 600 }}>{fmtUSD(item.usd)}</td>
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: '#7c85a2', margin: '4px 0 0' }}>Microsmart Servicio Técnico Apple</p>
        </div>
        ...
        <div style={{ position: 'relative', flex: 1, maxWidth: 380, minWidth: 200 }}>
          ...
          {search && (
            <button onClick={() => setSearch('')} style={{
              ...
              background: 'none', border: 'none', color: '#7c85a2', cursor: 'pointer', fontSize: 16, lineHeight: 1,
            }}>×</button>
          )}
        </div>
      </div>
      ...
      {/* Legend */}
      <div style={{ marginTop: 16, padding: '10px 14px', borderRadius: 8, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 20, fontSize: 12, color: '#7c85a2', flexWrap: 'wrap' }}>
      ...
      <style>{`input::placeholder{color:#475569}input:focus{border-color:#ec4899!important}`}</style>
    </div>
  )
}
